 ...
 * 
 * Adapted ConceptType comparator from AlphanumComparator
 * 
 * Adapted NaturalStringComparator from ConceptTypeComparator
 */
public class NaturalStringComparator implements Comparator<String> {
	 ...
	 * calculate it once)
	 **/
	private final String getChunk(final String s, final int slength, final int marker) {
		final StringBuilder chunk = new StringBuilder();
		int localMarker = marker;
		char c = s.charAt(localMarker);
		chunk.append(c);
		localMarker++;
		if (isDigit(c)) {
			while (localMarker < slength) {
				c = s.charAt(localMarker);
				if (!isDigit(c))
					break;
				chunk.append(c);
				localMarker++;
	...
	public int compare(final String arg0, final String arg1) {
		final String s1 = arg0;
		final String s2 = arg1;
