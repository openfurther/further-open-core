...
public class OtherExceptionMapper implements ExceptionMapper<Exception>
{
	 ...
	 */
	private static final Logger log = getLogger(OtherExceptionMapper.class);
	...
	public Response toResponse(final Exception exception)
	{
		...
		final ApplicationError entity = wrapByApplictionError(exception);
	 ...
	 * @return
	 */
	private ApplicationError wrapByApplictionError(final Exception exception)
	{
		if (ReflectionUtil.instanceOf(exception, ApplicationException.class))
		{
			final ApplicationException applicationException = (ApplicationException) exception;
			return applicationException.getError();
		}
		return new ApplicationError(ErrorCode.INTERNAL_ERROR, exception.getMessage());
	}
