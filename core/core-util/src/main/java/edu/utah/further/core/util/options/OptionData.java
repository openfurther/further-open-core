...
/**
 * This class holds all the data for an option. This includes the prefix, the key, the
 * separator (for value options), the multiplicity, and all the other settings describing
 * the option. The class is designed to be only a data container from a user perspective,
 * i. e. the user has read-access to any data determined by the {@link Options#check()},
 * but not access to any of the other methods which are used internally for the operation
 * of the actual check.
 * <p>
 * -----------------------------------------------------------------------------------<br>
...
public class OptionData
{
	...
	OptionData(final Options.Prefix prefix, final String key, final boolean detail,
	...
	/**
	 * Get the number of results found for this option, which is number of times the key
	 * matched
	 * <p>
	 *
	 ...
	 * {@link #getResultCount()}<code> - 1</code>. However, only for value options, a non-
	 * <code>null</code> value will be returned. Non-value options always return
	 * <code>null</code>.
	 * <p>
	 *
	 * @param index
	 ...
	 * {@link #getResultCount()}<code> - 1</code>. However, only for value options which
	 * take details, a non-<code>null</code> detail will be returned. Non-value options
