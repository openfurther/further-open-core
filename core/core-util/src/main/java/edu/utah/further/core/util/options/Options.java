...
/**
 * The central class for option processing. Sets are identified by their name, but there
 * is also an anonymous default set, which is very convenient if an application requires
 * only one set.
 * <p>
 * -----------------------------------------------------------------------------------<br>
...
public class Options
{
	...
	/**
	 * An enum encapsulating the possible separators between value options and their
	 * actual values.
	 */
	public enum Separator
	{
		...
		NONE('D'); // NONE is a placeholder in case no separator is required, 'D' is just
	...
	/**
	 * An enum encapsulating the possible prefixes identifying options (and separating
	 * them from command line data items)
	 */
	 ...
	 * @param prefix
	 *            The prefix to use for all command line options. It can only be set here
	 *            for all options at the same time
	 * @param defaultMultiplicity
	 *            The default multiplicity to use for all options (can be overridden when
	 *            adding an option)
	 * @param defMinData
	 *            The default minimum number of data items for all sets (can be overridden
	 *            when adding a set)
	 * @param defMaxData
	 *            The default maximum number of data items for all sets (can be overridden
	 *            when adding a set)
	 *            <p>
	 * @throws IllegalArgumentException
	 *             If either <code>args</code>, <code>prefix</code>, or
	 *             <code>defaultMultiplicity</code> is <code>null</code> - or if the data
	 ...
	 *            adding an option)
	 * @param data
	 *            The default minimum and maximum number of data items for all sets (can
	 *            be overridden when adding a set)
	 *            <p>
	 * @throws IllegalArgumentException
	 *             If either <code>args</code>, <code>prefix</code>, or
	 *             <code>defaultMultiplicity</code> is <code>null</code> - or if the data
	 ...
	 *            <p>
	 * @throws IllegalArgumentException
	 *             If either <code>args</code> or <code>defaultMultiplicity</code> is
	...
	/**
	 * Constructor. The prefix is set to {@link Prefix#DASH}, and the default number of
	 * data items is set to 0.
	 * <p>
	 *
	 * @param args
	 *            The command line arguments to check
	...
	/**
	 * Constructor. The prefix is set to {@link Prefix#DASH}, the default number of data
	 * items is set to 0, and the multiplicity is set to {@link Multiplicity#ONCE}.
	 * <p>
	 *
	 * @param args
	 *            The command line arguments to check
	...
	/**
	 * Constructor. The prefix is set to {@link Prefix#DASH}, and the multiplicity is set
	 * to {@link Multiplicity#ONCE}.
	 * <p>
	 *
	 * @param args
	 *            The command line arguments to check
	 ...
	 *            <p>
	 * @throws IllegalArgumentException
	 *             If <code>args</code> is <code>null</code> - or if the data range value
	 *             doesn't make sense
	 */
	 ...
	 *            <p>
	 * @throws IllegalArgumentException
	 *             If <code>args</code> is <code>null</code> - or if the data range values
	 *             don't make sense
	 */
	...
	/**
	 * Constructor. The default number of data items is set to 0, and the multiplicity is
	 * set to {@link Multiplicity#ONCE}.
	 * <p>
	 *
	 * @param args
	 *            The command line arguments to check
	 ...
	 *            <p>
	 * @throws IllegalArgumentException
	 *             If either <code>args</code> or <code>prefix</code> is <code>null</code>
	 */
	...
	public Options(final String args[], final Prefix prefix, final int defMinData,
			final int defMaxData)
	{
	...
	/**
	 * Return the (first) matching set. This invocation does not ignore unmatched options
	 * and requires that data items are the last ones on the command line.
	 * <p>
	 *
	 * @return The first set which matches (i. e. the <code>check()</code> method returns
	 *         <code>true</code>) - or <code>null</code>, if no set matches.
	 */
	 ...
	 *            A boolean to select whether unmatched options can be ignored in the
	 *            checks or not
	 * @param requireDataLast
	 *            A boolean to indicate whether the data items have to be the last ones on
	...
	{
		for (final String setName : optionSets.keySet())
	 ...
	 */
	public OptionSet addSet(final String setName, final int minData, final int maxData)
	{
		if (setName == null)
			throw new IllegalArgumentException(CLASS + ": setName may not be null");
		if (optionSets.containsKey(setName))
			throw new IllegalArgumentException(CLASS + ": a set with the name " + setName
					+ " has already been defined");
		final OptionSet os = new OptionSet(prefix, defaultMultiplicity, setName, minData,
				maxData);
	 ...
	 *            <p>
	 * @return The set to retrieve (or <code>null</code>, if no set with the given name
	 *         exists)
	 */
	 ...
	 * <code>check()</code> methods). This is useful to determine what was wrong with the
	 ...
	 * <code>false</code>, and <code>requireDataLast</code> is set to <code>true</code>.
	 * <p>
	 *
	...
	public boolean check(final boolean ignoreUnmatched, final boolean requireDataLast)
	{
	 ...
	 *         <p>
	 * @throws IllegalArgumentException
	 *             If either <code>setName</code> is <code>null</code>, or the set is
	 *             unknown.
	 */
			...
			final boolean requireDataLast)
	{
		...
		checkErrors = StringUtil.newStringBuilder();
		...
		final List<OptionData> options = set.getOptionData();
		final List<String> data = set.getData();
		final List<String> unmatched = set.getUnmatched();
				...
				if (m.lookingAt())
				{
					...
					{ // The code section for value options
						...
						{ // In this case, the next argument must be the value
							...
							{ // The last argument, thus no value follows it: Error
								checkErrors
							...
							else
							{
								...
								{ // The next one is an argument, not a value: Error
									checkErrors
	...
	/**
	 * Add the given value option to <i>all</i> known sets. See
	 * {@link OptionSet#addOption(String, Options.Separator, Options.Multiplicity)} for
	 * details.
	 */
	...
	/**
	 * Add the given value option to <i>all</i> known sets. See
	 * {@link OptionSet#addOption(String, boolean, Options.Separator)} for details.
	 */
	...
	{
		for (final String setName : optionSets.keySet())
			optionSets.get(setName)
					.addOption(key, details, separator, true, multiplicity);
	}

	/**
	 * This is the overloaded {@link Object#toString()} method, and it is provided mainly
